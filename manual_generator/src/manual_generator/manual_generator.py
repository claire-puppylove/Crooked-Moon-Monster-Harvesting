...
class Item:
    ...
    Attributes:
        ...
        effect:          str                : Effect or description of the item
    ...
    def __init__(
        ...
        monsters = [m.strip() for m in monsters.split(",")]
        ...
        self.ingredient_type = [ing.strip() for ing in ingredient_type.split(",")]
        self.crafting_value = [crf.strip() for crf in crafting_value.split(", ")]
        self.use = [u.strip() for u in use.split(",")]
        self.effect = effect
        self.cooking_effect = cooking_effect
        checks = [fch.strip() for fch in find_checks.split(",")]
        self.find_checks = {
            monster:[
        ...
        self.location = {
            monster:[
                ...
                if f"({monster})" in loc or len(loc.split(")"))<3
                ]
            for monster in self.monsters
        }
